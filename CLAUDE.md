# obsidian-custom-open

Plugin Obsidian. **Le code vit ici, jamais dans le vault.**

| | |
|---|---|
| Dossier source | `code/jdr/obsidian-plugins/obsidian-custom-open/` |
| Branche de travail | `staging` |
| Identifiant du plugin | `obsidian-custom-open` |
| Cible de déploiement | `<vault>/.obsidian/plugins/obsidian-custom-open/` |
...
```powershell
..\..\obsidian-plugin-tools\deploy-plugin.ps1 obsidian-custom-open     # Windows
```

```bash
../../obsidian-plugin-tools/deploy-plugin.sh obsidian-custom-open      # macOS
```
...
- **Ne jamais éditer les fichiers dans `.obsidian/plugins/obsidian-custom-open/`.** C'est une cible
